...
export interface GlassNavItem {
  /** Unique value bound via v-model */
  value: string | number
  /** Visible label (hidden when an icon slot is used, but kept for a11y) */
  label: string
}
...
// Direction-aware transform-origin for the squish animation.
...
<template>
  <div ref="rootEl" class="glass-nav" role="tablist" :aria-label="ariaLabel">
    <!-- Sliding liquid-glass highlight -->
    <span
      :key="animKey"
      class="glass-nav__indicator"
      :style="{
    ...
    <label
      ...
      :key="item.value"
      ref="optionEls"
      class="glass-nav__option"
      :class="{ 'is-active': item.value === modelValue }"
    >
      <input
        class="glass-nav__input"
        type="radio"
      ...
      />
      <span class="glass-nav__content">
        <slot name="icon" :item="item" :active="item.value === modelValue">{{ item.label }}</slot>
      </span>
    </label>

    <!-- SVG displacement filter powering the glass refraction -->
    <div class="glass-nav__filter" aria-hidden="true">
...
<style scoped>
.glass-nav {
  /* Glass palette — override these on the host to retheme. */
  --c-glass: #bbbbbc;
  --c-light: #fff;
  --c-dark: #000;
  --c-content: #fff;
  --c-action: var(--color-primary, #0052f5);
  --glass-reflex-dark: 1;
  ...
  backdrop-filter: blur(8px) url(#liquid-glass-nav) saturate(var(--saturation));
...
/* Dark variant from the original pen — delicate edges, light content.
   Kicks in automatically to match daisyUI's dark theme. */
...
.glass-nav__option:hover {
  --c: var(--c-action);
  cursor: pointer;
}

.glass-nav__option.is-active {
  --c: var(--c-action);
  cursor: auto;
}
...
.glass-nav__content :deep(svg) {
  display: block;
  width: 26px;
  height: 26px;
}

/* Selected icon sits on the bright glass highlight — a solid (filled) icon
   plus a soft shadow keeps it crisp and clearly recognizable. */
.glass-nav__option.is-active .glass-nav__content :deep(svg) {
  filter: drop-shadow(0 1px 1.5px color-mix(in srgb, var(--c-dark) 45%, transparent));
}
...
.glass-nav__indicator {
  ...
  border-radius: 99em;
  /* Subtly brighter frosted glass — distinct from the bar without going stark white. */
  ...
  transition:
    left 400ms cubic-bezier(1, 0, 0.4, 1),
    width 400ms cubic-bezier(1, 0, 0.4, 1),
    background-color 400ms cubic-bezier(1, 0, 0.4, 1),
    box-shadow 400ms cubic-bezier(1, 0, 0.4, 1);
  animation: glassSquish 440ms ease;
}

@keyframes glassSquish {
  0% { scale: 1 1; }
  50% { scale: 1.15 1; }
  100% { scale: 1 1; }
}
